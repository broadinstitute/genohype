...
/**
 * Time-series chart panel showing memory utilization percentage per worker.
 */
export const MemoryChartPanel: React.FC = () => {
  ...
  const chartData = useMemo(() => {
    ...
    const datasets = metrics.workers.map((worker, i) => ({
      ...
      data: worker.snapshots.map((s) => {
        if (s.memory_used_bytes && s.memory_total_bytes && s.memory_total_bytes > 0) {
  ...
  return (
    <div className="panel-container" style={{ display: 'flex', flexDirection: 'column' }}>
      <h2 className="panel-title">Memory Utilization (%)</h2>
...
export default MemoryChartPanel;
